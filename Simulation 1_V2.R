  ...
  for(k in 1:length(sample.size)){
    ...
    for(j in 1:Nsims){
      ...
      J_results <- bind_rows(J_results,
                             ...
                             Poisson.test(data.cells=data.cells,group=group,ncell=ncell,N=N),
                             MWU.test(ncell=ncell,data.proportions=data.proportions,group=group,N=N))
    }
...
abund_low_effect_small <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
abund_mod_effect_small <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
abund_mod_effect_small <- abund_mod_effect_small %>% mutate(Abundance="Moderate Abund Cell")

### High Abundance Cell Type ----
lambda1 = c(250,250,250,250,500,500,500,500,1000,1000,1000,1000,1000)
lambda2 = c(250,250,250,250,500,500,500,500,1050,1000,1000,1000,1000)
abund_high_effect_small <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
abund_low_effect_medium <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
abund_mod_effect_medium <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
abund_high_effect_medium <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
abund_low_effect_large <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
abund_mod_effect_large <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
abund_mod_effect_large <- abund_mod_effect_large %>% mutate(Abundance="Moderate Abund Cell")

### High Abundance Cell Type ----
lambda1 = c(250,250,250,250,250,500500,500,500,500,1000,1000,1000,1000,1000)
lambda2 = c(250,250,250,250,250,500500,500,500,500,1150,1000,1000,1000,1000)
abund_high_effect_large <- Simulation(Nsims=Nsims,sample.size=sample.size,lambda1=lambda1,lambda2=lambda2,W.star=W.star,ncell=ncell)
...
# SAVING RESULTS -----
if(Outputs==TRUE){
  openxlsx::write.xlsx(data.frame(Full_Results),paste0(Outputs_path,"Sim1_Results.xlsx"))
}
